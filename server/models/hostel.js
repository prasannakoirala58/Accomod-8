    ...
    rooms: [
      {
        ...
        room_type: {
          type: String,
          default: 'three-seater',
          enum: ['one_seater', 'two_seater', 'three_seater', 'four_seater'],
          required: true,
        },
        availability: {
          type: Boolean,
...
hostelSchema.methods.compute_rating = function (overall_rating) {
  this.hostel_rating =
    (this.hostel_rating * (this.reviews.length - 1) + overall_rating) / this.reviews.length;
...
hostelSchema.methods.compute_rating_after_delete = function (overall_rating) {
  this.hostel_rating =
    (this.hostel_rating * this.reviews.length - overall_rating) / (this.reviews.length - 1);
